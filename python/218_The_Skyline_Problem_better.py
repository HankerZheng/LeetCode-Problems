...
# Key Points: Traverse all the buildings from the left to right and
#             push the building's right line into a heap sorted
#             according to its height.
#
#             If a new building's left line is less than the max height's
#             right line. Pop the right line out.
#
#             Add new point into ANS only when it is the first point or
#             the max height has changed.
#
# Time Complexity: NloN
#                  This method is one sort time more quick than the previous one
#
# Run time: 120 ms
    ...
    def getSkyline(self, LRH):
        ...
        while i < n or liveHR:
            if not liveHR or i < n and LRH[i][0] <= -liveHR[0][1]:
                # When liveHR is empty or
                # When current building's LEFT is less than the highest building's RIGHT
                x = LRH[i][0] # Store the left position
                # This loop is for buidings with same x-left axis
                while i < n and LRH[i][0] == x:
                    heappush(liveHR, (-LRH[i][2], -LRH[i][1]))
                    i += 1
            else:
                # When no new buildings or
                # current building's LEFT is larger than the highest building's RIGHT
                x = -liveHR[0][1] # Store the right position
                while liveHR and -liveHR[0][1] <= x:
                    # Pop out all the ending line that is before this x
                    heappop(liveHR)
            # height is 0 or max_height
            height = len(liveHR) and -liveHR[0][0]
            if not skyline or height != skyline[-1][1]:
                # When skyline is empty or height is not equal to last height
                # in the skyline, which is for the ending line with the same
                # right x-axis value
                skyline += [x, height],
        return skyline
